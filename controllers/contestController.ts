...
export const inactiveContest=async (req:Request, res:Response) => {
  try{
    const page = parseInt(req.query.page as string) || 1;
    const limit = parseInt(req.query.limit as string) || 10;
    const skip = (page - 1) * limit;
...
export const getLeaderboardByContests=async (req:Request, res:Response) => {
  try {
    const { contestId } = req.params;
    if (!contestId) {
      return res.status(404).json({
...
export const mappingId=async (req:Request,res:Response)=>{
  try{
